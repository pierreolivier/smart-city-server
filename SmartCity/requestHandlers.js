var fs = require('fs'),
	...
	//multipart = require("./node_modules/multipart/lib/multipart.js"),
...
// 	jade.renderFile(__dirname + '/jade/home.jade', options, function (err, html) {
//     	console.log(html);
// 	});
// }
...
function maj(response, request){
	...
	var url_parts = url.parse(request.url, true),
		...
		con ="postgres://eric:cire@"+domain+":5432/gis",
		rows = [],
		myArray = [],
	...
	// 	bool = true;
	// 	req = "SELECT longitude, latitude, gid, etat from declaration d, citoyens c WHERE longitude BETWEEN "+query.minlng+" AND "+query.maxlng+" AND latitude BETWEEN "+query.minlat+" AND "+query.maxlat+" AND etat & B'"+query.etat+"'=etat AND login='"+query.login+"' AND mdp='"+query.mdp+"' AND d.idcitoyen = c.id_citoyen";
	// }
	//login = getLogin(request);


	//moyen très mauvais de détecter l'utilisation de services via l'application
	if (query.app){
		bool = true;
		console.log("application");
		req = "SELECT longitude, latitude, gid, etat from declaration WHERE longitude BETWEEN "+query.minlng+" AND "+query.maxlng+" AND latitude BETWEEN "+query.minlat+" AND "+query.maxlat+" AND etat & B'"+query.etat+"'=etat";
	}
	//si infos_login[login] == 2, alors on est admin, et on accès à toutes les données de sa ville!!!!!!!!!
	//pour l'instant un admin a acces à l'ensemble des données de la BDD => rajouter idville pour chaque déclaration
	else if (infos_login[getLogin(request)+'.role'] == 'admin' ){
				bool = true;
		console.log("admin");
		req = "SELECT * from declaration d, incidents i, personnel p WHERE d.longitude BETWEEN "+query.minlng+" AND "+query.maxlng+" AND d.latitude BETWEEN "+query.minlat+" AND "+query.maxlat+" AND d.etat & B'"+query.etat+"'=d.etat AND i.id_type = d.type AND d.id_ville = p.idville and p.login ='"+getLogin(request)+"' AND p.role = 'admin'";
	}
	else if (infos_login[getLogin(request)+'.id'] !== undefined ){
		bool = true;
		console.log("portail");
		req = "SELECT * from declaration d, incidents i WHERE d.longitude BETWEEN "+query.minlng+" AND "+query.maxlng+" AND d.latitude BETWEEN "+query.minlat+" AND "+query.maxlat+" AND d.etat & B'"+query.etat+"'=d.etat AND i.id_type = d.type AND persaffectee="+infos_login[getLogin(request)+'.id'];
	...
	//req = "SELECT ST_X(the_geom), ST_Y(the_geom), gid, etat from declaration WHERE ST_MakeEnvelope("+query.minlng+","+query.minlat+","+query.maxlng+","+query.maxlat+",4326) && the_geom AND etat & B'"+query.etat+"'=etat";
		console.log(req);  
...
function setEtat(response, gid, value){
	console.log("setEtat");
	var con ="postgres://eric:cire@"+domain+":5432/gis";
	...
	query.on('end', function(result){
		if (rows.length !=1) {
         ...
         'Access-Control-Allow-Credentials': 'true'});
	        ...
	        myArray.join(',');
		  	client.end();
	  		//window.location.replace('http://"+domain+":8887/')
	  		//var rs = fs.createReadStream('./leaflet.html');
        	//util.pump(rs);
        	response.writeHead(200, {"Content-Type": "application/json", 'Access-Control-Allow-Origin': '*',
         'Access-Control-Allow-Credentials': 'true'})
...
function upload_complete(res,gid,ext,name) {
	...
	if (is_num(parseInt(gid))){
		...
		console.log(ext);
		/*if (ext === 'png' || ext === 'jpg'){
			req ="INSERT INTO \"public\".\"photos\" (\"idphoto\",\"iddeclaration\",\"nom\") VALUES ("+val+",'"+gid+"','"+name+"')";
		}
		else if (ext === 'mp4' || ext === 'mov'){
			req ="UPDATE \"public\".\"declaration\" SET video='"+name+"' WHERE gid="+gid;
		}
		else if (ext === 'mp3' || ext === 'wav' || ext === 'm4a' || ext === '3gpp'){
			req ="UPDATE \"public\".\"declaration\" SET son='"+name+"' WHERE gid="+gid;
		}*/
		if (ext === 'png' || ext === 'jpg'){
...
function getDataTable(response, request){
    ...
    if (request.method == 'POST'){	
		...
		var sequelize = new Sequelize('gis', 'eric', 'cire', {
		       dialect: 'postgres',
		       port: 5432,
